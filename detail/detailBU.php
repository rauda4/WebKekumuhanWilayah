	...
	?>
		...
		<div>
			...
			<table class="table table-bordered">
				...
				<br>$catatan</td>
				<td><div align='center'>
<a href='?mnu=detail&pro=ubah&kode=$id_detail&id=$id_penilaian'><img src='ypathicon/ub.png' title='ubah'></a>
...
onClick='return confirm(\"Apakah Anda benar-benar akan menghapus $id_penilaian pada data penilaian ?..\")'></a></div></td>
		...
		if ($jmldata > 0) {
			...
			if ($page > 1) {
				$prev = $page - 1;
				echo "<span class=prevnext><a href='$_SERVER[PHP_SELF]?page=$prev&mnu=detail&id=<?php echo $id_penilaian; ?>'>« Prev</a></span> ";
			} else {
			...
			for ($i = 1; $i <= $jmlhal; $i++)
				if ($i != $page) {
					echo "<a href='$_SERVER[PHP_SELF]?page=$i&mnu=detail&id=<?php echo $id_penilaian; ?>'>$i</a> ";
				} else {
			...
			if ($page < $jmlhal) {
				$next = $page + 1;
				echo "<span class=prevnext><a href='$_SERVER[PHP_SELF]?page=$next&mnu=detail&id=<?php echo $id_penilaian; ?>'>Next »</a></span>";
			} else {
...
'$catatan'
				...
				if ($simpan) {
					echo "<script>alert('Data $id_penilaian berhasil disimpan !');document.location.href='?mnu=detail&id=$id_penilaian';</script>";
				} else {
					echo "<script>alert('Data $id_penilaian gagal disimpan...');document.location.href='?mnu=detail&id=$id_penilaian';</script>";
				}
			} else {
	...
	`catatan`='$catatan'
	 WHERE `id_detail`='$id_detail0'";
				...
				if ($ubah) {
					echo "<script>alert('Data $id_penilaian berhasil diubah !');document.location.href='?mnu=detail&id=$id_penilaian';</script>";
				} else {
					echo "<script>alert('Data $id_penilaian gagal diubah...');document.location.href='?mnu=detail&id=$id_penilaian';</script>";
				}
			} //else simpan
		}
		?>
		...
		if (isset($_GET["pro"]) && $_GET["pro"] == "hapus") {
			...
			if ($hapus) {
				echo "<script>alert('Data $id_detail berhasil dihapus !');document.location.href='?mnu=detail&id=$id_penilaian';</script>";
			} else {
				echo "<script>alert('Data $id_detail gagal dihapus...');document.location.href='?mnu=detail&id=$id_penilaian';</script>";
			}
		}
		?>