...
<div id="accordion">
	...
	<div>
		...
		<form action="" method="post" enctype="multipart/form-data">
			<table class="table">
				<tr>
					<th width="20%"><label for="id_kelurahan">ID</label>
					<th width="1%">:
					<th colspan="2"><b><?php echo $id_kelurahan; ?></b>
				</tr>

				<tr>
					<td><label for="nama_kelurahan">Nama Kecamatan/Kelurahan/Rt Rw</label>
					<td>:
				...
				<tr>
					...
					<td colspan="2">
						...
						<a href="?mnu=kelurahan"><input class="btn btn-warning" name="Batal" type="button" id="Batal" value="Batal" /></a>
					</td>
				</tr>
			</table>
		</form>
	...
	<h3>Data Kecamatan/Kelurahan/Rt Rw :</h3>
	<div>
		...
		<table class="table table-bordered">
			...
			if ($jum > 0) {
				...
				<td>
				<b>$nama_kelurahan</b>
				<br><small><i>$deskripsi</i></small></td>
				<td><div align='center'>
<a href='?mnu=kelurahan&pro=ubah&kode=$id_kelurahan'><img src='ypathicon/ub.png' title='ubah'></a>
<a href='?mnu=kelurahan&pro=hapus&kode=$id_kelurahan&nama_kelurahan=$nama_kelurahan'><img src='ypathicon/ha.png' title='hapus' 
onClick='return confirm(\"Apakah Anda benar-benar akan menghapus $nama_kelurahan pada data kelurahan ?..\")'></a></div></td>
		...
		if ($jmldata > 0) {
			...
			if ($page > 1) {
				$prev = $page - 1;
				echo "<span class=prevnext><a href='$_SERVER[PHP_SELF]?page=$prev&mnu=kelurahan'>« Prev</a></span> ";
			} else {
			...
			for ($i = 1; $i <= $jmlhal; $i++)
				if ($i != $page) {
					echo "<a href='$_SERVER[PHP_SELF]?page=$i&mnu=kelurahan'>$i</a> ";
				} else {
			...
			if ($page < $jmlhal) {
				$next = $page + 1;
				echo "<span class=prevnext><a href='$_SERVER[PHP_SELF]?page=$next&mnu=kelurahan'>Next »</a></span>";
			} else {
...
'$deskripsi'
			...
			if ($simpan) {
				echo "<script>alert('Data $nama_kelurahan berhasil disimpan !');document.location.href='?mnu=kelurahan';</script>";
			} else {
				echo "<script>alert('Data $nama_kelurahan gagal disimpan...');document.location.href='?mnu=kelurahan';</script>";
			}
		} else {
	...
	`deskripsi`='$deskripsi'
	 WHERE `id_kelurahan`='$id_kelurahan0'";
			...
			if ($ubah) {
				echo "<script>alert('Data $nama_kelurahan berhasil diubah !');document.location.href='?mnu=kelurahan';</script>";
			} else {
				echo "<script>alert('Data $nama_kelurahan gagal diubah...');document.location.href='?mnu=kelurahan';</script>";
			}
		} //else simpan
	}
	?>
	...
	if (isset($_GET["pro"]) && $_GET["pro"] == "hapus") {
		...
		if ($hapus) {
			echo "<script>alert('Data $nama_kelurahan berhasil dihapus !');document.location.href='?mnu=kelurahan';</script>";
		} else {
			echo "<script>alert('Data $nama_kelurahan gagal dihapus...');document.location.href='?mnu=kelurahan';</script>";
		}
	}
	?>